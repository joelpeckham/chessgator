...
export const PRODUCTS: readonly ProductNode[] = [
  ...
  },
  {
    ...
    description: "Independent University of Wyoming class schedule planner.",
  },
  {
    id: "qr",
    ...
    description: "Free QR code generator. Export real SVG and PNG. No paywall.",
  },
];
...
export function alsoByJoelMarkdown(excludeId?: ProductId): string {
  ...
  return [
    ...
    ...products.map(
      (p) =>
        `- [${p.name}](${p.url}) — ${p.description} — [llms.txt](${p.llms})`,
    ),
    "",
    makerHiringMarkdown(),
  ].join("\n");
